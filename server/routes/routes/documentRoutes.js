...
router.delete("/mypost/:postId", protect, deletePost); //게시물 삭제
router.patch("/:postId", protect, updatePost); //게시물 수정
router.get("/:postId", protect, viewPost); //다른 사람 게시물 조회
router.get("/:postId", protect, joinParty); //파티 참여(다른유저에게 알림보내기)
router.delete("/:postId", protect, leaveParty); //파티 떠나기(다른유저에게 알림보내기)
